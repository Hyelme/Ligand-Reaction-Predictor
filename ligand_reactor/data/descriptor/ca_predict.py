...
x2=np.array(csv1.values)[:,0:3]
#print(X.shape)
x4=[]

new_model = tf.keras.models.load_model("C:\APM_Setup\htdocs\ligand_reactor\data\descriptor\A_Plan_cnn.h5")
#X[y,x,z] 형태
#print(X[0,:,:])
#print(X[1,:,0])
...
f=open("C:\APM_Setup\htdocs\ligand_reactor\data\predict\CA_plan.txt", mode='wt')
#y_pred =[]
for i in range(0,len(X)):
  #  print(X[i,3:,:].reshape(1,-1))
    ...
    f.write(format(csv2['Label'].unique()[new_model.predict_classes(np.array(x))]).replace('lhr_Agonist','nhr_Agonist').replace('nhr_Inhibition','nhr_Inhibition'))
    f.writelines('\n')
