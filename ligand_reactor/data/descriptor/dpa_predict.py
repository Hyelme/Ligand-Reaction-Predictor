...
csv1 = pd.read_csv('C:\APM_Setup\htdocs\ligand_reactor\data\descriptor\P_Action_Descript.txt')  #test data
csv2 = pd.read_csv('C:\APM_Setup\htdocs\ligand_reactor\data\descriptor\P_action_Label.txt')  #label data
# csv 파일 열기
...
#print(Y)
X = np.array(csv1.values)[:,2:618].astype(float)

x2=np.array(csv1.values)[:,1:2]


#cnn_aplan_model.h5
new_model = tf.keras.models.load_model('C:\APM_Setup\htdocs\ligand_reactor\data\descriptor\P_action_dmlp.h5')
#X[y,x,z] 형태
#print(X[0,:,:])
#print(X[1,:,0])
...
f=open('C:\APM_Setup\htdocs\ligand_reactor\data\predict\DPA_plan.txt', mode='wt')
for i in range(0, len(X)):
    ...
    f.write(format(csv2['Label'].unique()[new_model.predict_classes(np.array(x))]))
    f.writelines('\n')
