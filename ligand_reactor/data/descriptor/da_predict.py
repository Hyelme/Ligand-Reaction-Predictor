...
csv1 = pd.read_csv('C:\APM_Setup\htdocs\ligand_reactor\data\descriptor\A_Descript.txt')  #test data
csv2 = pd.read_csv('C:\APM_Setup\htdocs\ligand_reactor\data\descriptor\A_Plan_Label.txt')  #label data
# csv 파일 열기
...
#print(Y)
X = np.array(csv1.values)[:,2:993].astype(float)

x2=np.array(csv1.values)[:,2:3]


#cnn_aplan_model.h5
new_model = tf.keras.models.load_model('C:\APM_Setup\htdocs\ligand_reactor\data\descriptor\A_Plan_dmlp.h5')
#X[y,x,z] 형태
#print(X[0,:,:])
#print(X[1,:,0])
...
f=open('C:\APM_Setup\htdocs\ligand_reactor\data\predict\DA_plan.txt', mode='wt')
for i in range(0, len(X)):
    ...
    f.write(format(csv2['Label'].unique()[new_model.predict_classes(np.array(x))]).replace('lhr_Agonist','nhr_Agonist').replace('lhr_Inhibition','nhr_Inhibition'))
    f.writelines('\n')
